...
MT is trying to make sure that the functions in this module are multiprocessing-safe and thread-safe.
"""
...
def store_exec(func, store, *args, **kwds):
...
def keys(store):
    ...
    """
    return store_exec(
        lambda store: [key[5:] for key in store.keys() if key.startswith("item_")],
        store,
    )
