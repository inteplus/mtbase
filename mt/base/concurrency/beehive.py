...
class Bee:
    ...
    def __init__(
        ...
        # task scheduling
        self.pending_order_map = (
            {}
        )  # task_id -> (timestamp, candidate_children_set), orders that have been placed but not yet accepted
        self.assigned_order_map = (
            {}
        )  # task_id -> child_id, orders that have been accepted but not yet awaited by the parent bee
    ...
    async def run(self):
        ...
        while (
            (not self.to_terminate)
            or (self.pending_task_cnt > 0)
            or self.towork_task_list
            or self.working_task_map
        ):
    ...
    def _put_msg(self, child_id, msg):
        ...
        conn = (
            self.p_m2p if child_id < 0 else self.child_conn_list[child_id][1]
        )  # child case: m2c
        try:
            conn.put_nowait(msg)
        except:
    ...
    def _dispatch_new_child_msg(self, child_id, msg):
        ...
        if msg_type == "dead":
            ...
            if task_id in self.pending_order_map:
                ...
                self._put_msg(
                    child_id, {"msg_type": "task_taken", "task_id": task_id}
                )  # tell the child the task has been taken
    ...
    def _dispatch_new_parent_msg(self, msg):
        ...
        if msg_type == "die":  # request to die?
            ...
            if (not self.to_terminate) and (
                ...
                self._put_msg(
                    -1, {"msg_type": "task_accepted", "task_id": task_id}
                )  # notify the parent
                self.pending_task_cnt += 1  # awaiting further info
            else:
                self._put_msg(
                    -1, {"msg_type": "task_denied", "task_id": task_id}
                )  # notify the parent
            ...
            task = asyncio.ensure_future(
                self._execute_task(
                    name=msg["name"], args=msg["args"], kwargs=msg["kwargs"]
                )
            )
...
class WorkerBee(Bee):
    ...
    async def busy_status(self, context_vars: dict = {}):
        ...
        return (
            ...
            else len(self.working_task_map) / self.max_concurrency
        )
    ...
    def __del__(self):
        for process in self.process_map:
            ...
            if process.exitcode is not None:
                if self.logger:
                    self.logger.warn(
                        "Subprocess {} has terminated with exit code {}.".format(
        ...
        while not self.stop_managing and not self.to_terminate:
            ...
            for child_id in range(len(self.child_conn_list)):
                ...
                if not self.process_map[
                    child_id
                ].is_alive():  # killed without sending the last wish?
                    msg = {
                        "msg_type": "dead",
                        "death_type": "killed",
                        "exception": RuntimeError(
                            "Child bee id {} has died without sending a last wish.".format(
                                child_id
                            )
                        ),
                        "traceback": extract_stack_compact(),
...
async def beehive_run(
    ...
    if msg["status"] == "cancelled":
        raise asyncio.CancelledError(
            "The queen bee cancelled the task. Reason: {}".format(msg["reason"])
        )
