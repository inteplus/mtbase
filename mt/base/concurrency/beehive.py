...
class Bee:
    ...
    async def handle_task_message(self, conn_id: int, msg_id: int, task_info: dict):
    ...
    async def run(self):
        ...
        while (self.await_new_msg_task is not None) or self.pending_request_list or self.working_task_map:
            ...
            for i in range(num_requests):
                ...
                task = asyncio.ensure_future(self.handle_task_message(conn_id, msg_id, task_info))
                self.working_task_map[task] = (conn_id, msg_id)
    ...
    def process_done_await_new_msg(self, task):
        ...
        if msg_type == 'die': # request to die?
            ...
            self.process_death_wish(conn_id, msg)
