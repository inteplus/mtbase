...
def on_list(func):
    ...
    ----------
    func : function
        the function to build upon. Its expected form is `def func(x, *args, **kwds) -> object`.

    Returns
    -------
    function
        a wrapper function `def func_on_list(list_x, *args, **kwds) -> list_of_objects` that invokes `func(x, *args, **kwds)` for each x in list_x.
    """

    def func_on_list(list_x, *args, **kwds):
        return [func(x, *args, **kwds) for x in list_x]
